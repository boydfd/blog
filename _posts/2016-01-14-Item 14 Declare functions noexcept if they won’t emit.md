---
layout:     post
title:      '如果函数不会抛出异常就把它们声明为noexcept'
author:     boydfd
tags:       effective-mordern-C++ C++
subtitle:   'Item 14:如果函数不会抛出异常就把它们声明为noexcept'
...
对于一些函数，情况变得更加强烈（更多的优化）。move操作就是一个很好的例子。假设你有一份C++98代码，它使用了std::vector<Widget>。Widget通过一次次push_back来加到std::vector中:
